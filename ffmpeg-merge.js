 ...
 * Tries: 1) ffprobe-static, 2) derive from ffmpeg path, 3) system ffprobe
 */
function getFfprobePath() {
  try {
    ...
    if (ffmpegPath && ffmpegPath !== 'ffmpeg') {
      const derived = ffmpegPath.replace(/ffmpeg$/, 'ffprobe');
...
async function checkFfmpegVersion() {
  return new Promise((resolve) => {
    ...
    ffmpeg.on('close', (code) => {
      if (code === 0) {
        // Parse version from first line (e.g., "ffmpeg version 4.4.0 ...")
        const versionMatch = output.match(/ffmpeg version\s+([\d.]+)/);
...
/**
 * Verify audio stream in a media file
 * @param {string} filePath - Path to media file
 * @returns {Promise<object>} - { hasAudio: boolean, details: string }
 */
async function verifyAudioStream(filePath) {
  const basename = path.basename(filePath);

  // First, try using ffprobe
  return new Promise((resolve) => {
    ...
    ffprobe.on('close', (code) => {
      if (code === 0) {
        const hasAudio = output.trim().length > 0 && output.includes('audio');
        console.log(`[AudioCheck] file=${basename} audio_stream=${hasAudio}`);
        resolve({
          hasAudio: hasAudio,
          details: `Audio stream: ${hasAudio ? 'present' : 'not found'}`
        });
      } else {
        // Fall back to parsing ffmpeg stderr
        fallbackAudioCheck(filePath, resolve, basename);
      }
    });

    ffprobe.on('error', () => {
      // ffprobe not available, fall back to ffmpeg parsing
      fallbackAudioCheck(filePath, resolve, basename);
    });
  });
}
...
/**
 * Fallback audio check using ffmpeg when ffprobe is unavailable
 */
function fallbackAudioCheck(filePath, resolve, basename) {
  const ffmpeg = spawn(getFfmpegPath(), ['-i', filePath]);
  let stderr = '';
  ...
  ffmpeg.on('close', () => {
    // Parse stderr for "Audio:" line
    const audioMatch = stderr.match(/Audio:\s+/);
  ...
  ffmpeg.on('error', () => {
    console.log(`[AudioCheck] file=${basename} audio_stream=false`);
    resolve({
      hasAudio: false,
      details: 'Unable to verify (ffmpeg error)'
    });
  });
}
...
async function mergeVideoAudio(videoPath, audioPath, options = {}) {
  ...
  console.log(`[FFMPEG]   Video: ${path.basename(videoPath)}`);
  console.log(`[FFMPEG]   Audio: ${path.basename(audioPath)}`);
  console.log(`[FFMPEG]   Output: ${path.basename(finalOutputPath)}`);

  // Get durations for metadata
  const videoDuration = await getVideoDuration(videoPath);
  const audioDuration = await getAudioDuration(audioPath);

  console.log(`[FFMPEG]   Video duration: ${videoDuration?.toFixed(2) || 'unknown'}s`);
  console.log(`[FFMPEG]   Audio duration: ${audioDuration?.toFixed(2) || 'unknown'}s`);
  ...
  return new Promise((resolve, reject) => {
    ...
    ffmpeg.stderr.on('data', (data) => {
      ...
      if (progressMatch) {
        process.stdout.write(`\r[FFMPEG] Progress: ${progressMatch[1]}`);
      }
    });
    ...
    ffmpeg.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        console.log(`[FFMPEG] Merge complete! Output: ${fileSizeMB} MB`);
        ...
        console.error('[FFMPEG] ffmpeg error:', stderr.slice(-500));
...
async function batchMerge(pairs, options = {}) {
  console.log(`[FFMPEG] Batch merging ${pairs.length} video/audio pairs...`);

  const results = [];

  for (let i = 0; i < pairs.length; i++) {
    const { videoPath, audioPath, outputFilename } = pairs[i];
    console.log(`\n[FFMPEG] Merging ${i + 1}/${pairs.length}...`);
    ...
    try {
      ...
      console.error(`[FFMPEG] Merge failed for pair ${i + 1}:`, error.message);
      results.push({
        success: false,
        error: error.message,
        videoPath,
  ...
  console.log(`\n[FFMPEG] Batch merge complete: ${successful}/${pairs.length} successful`);
